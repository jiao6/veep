...
$SQL = "SELECT L.* FROM LESSON L
	where  L.ID = '$lessonId'
	and L.STATUS=". $STATUS_EFFECTIVE ." AND L.SHOWN=". $SHOWN_YES ." ";
//echo $SQL . "<br/>";
$dsql = new DSQL();
$dsql->query($SQL);
...
?>
    <div class="contain test">
        <div class="ts_top">
            ...
            <div class="tst_info">
                ...
                <p>
                    <br/><br/>
                    <?
                    $SQL = "SELECT * FROM coursesemail where coursesid='$lessonId' and email = '$auth_email'";
                    $dsql->query($SQL);//批量导入的用户  白名单用户
                    if ($dsql->next_record()) {
                        $iscoursesemail = 1;
                    }
                    $SQL = "SELECT * FROM coursesuser where coursesid='$lessonId' and userid = '$auth_id' and status=" . $STATUS_EFFECTIVE;
                    $dsql->query($SQL);
                    if ($dsql->next_record()) {//选课码进入的用户
                        ...
                        if ($dsql->next_record($SQL)) {
                            ...
                            $dsql->next_record();
                            $allcount = $dsql->f("c");
                            if (1==1||$student_limit > $allcount) {//取消课堂上线
                                $SQL = "SELECT  ifnull( trueusage ,0) as trueusage,payquantity FROM courses where  id  ='$course_id'";
                                //echo $SQL . "<br/>";
                                $dsql->query($SQL);
                                $dsql->next_record();
                                ...
                                if ($payquantity > $trueusage) {
                                    ...
                                    echo "  <script >alert('加入人次 $trueusage 超过上限 $payquantity ！')</script>\n";
                                }
                            } else {
                                echo "  <script >alert('加入人次 $allcount 超过上限 $student_limit ！')</script>\n";
                            }
                        }
                        if ($auth_pid == 1) {
                ...
                <form method="post" style="float: right;top:100px; right: 5px;font-size: 15px">
        ...
        $dsql->next_record();
        $allcount = $dsql->f("c");
        $dsql->query("UPDATE LESSON SET STUDENT_AMOUNT='$allcount'  WHERE  ID='$lessonId'");
        //echo "update lesson set student_amount='$allcount'  where  id='$lessonId'";

        $SQL = "SELECT  IFNULL(SUM(STUDENT_AMOUNT),0) AS TRUEUSAGE FROM LESSON WHERE  COURSE_ID  ='$course_id' ";
        //echo $SQL . "<br/>";
        $dsql->query($SQL);
        $dsql->next_record();
        $trueusage = $dsql->f("TRUEUSAGE");
        $dsql->query("update courses set trueusage='$trueusage' where   id  ='$course_id'");
        ...
        $SQL = "select id from coursesbbs where coursesid= '$lessonId' and userid='$auth_id' and content='$bbscontent'";
        $dsql->query($SQL);
        if (!$dsql->next_record()) {
            $SQL = "insert into coursesbbs( id,coursesid,userid,content,createtime)values(0,'$lessonId','$auth_id','$bbscontent',now())";
            $dsql->query($SQL);
        } else {
            ...
            if ($ac == "addteacher") {
                $SQL = "SELECT * FROM coursesuser where coursesid='$lessonId'  and  userid = '$auth_id'  and isbook=1";
                //echo $SQL . "<br/>";
                $dsql->query($SQL);
                if (!$dsql->next_record()) {
                    $SQL = " update  coursesuser  set teacheremail = '$teacheremail'  where coursesid='$lessonId'  and  userid = '$auth_id' and isbook=1";
                    $dsql->query($SQL);
                    if (!$dsql->query($SQL)) {
                        echo "  <script >alert('绑定老师失败！')</script>\n";
                    } else {
                ...
                if ($code != $usercode) {
                    $SQL = "SELECT * FROM coursescode where coursesid='$lessonId'  and  code = '$code'  and endtime >now() and status=0";
                    $dsql->query($SQL);
                    if ($dsql->next_record()) {
                        $day = $dsql->f("day");
                        if (!intval($day * 1) > 0) $day = 0;
                        $SQL = "update coursescode  set status=1 where coursesid='$lessonId'  and  code = '$code'  and endtime >now()";
                        $dsql->query($SQL);
                        $SQL = "SELECT * FROM coursesuser where coursesid='$lessonId'  and  userid = '$auth_id' ";
                        //echo $SQL . "<br/>";
                        $dsql->query($SQL);
                        if (!$dsql->next_record()) {
                            ...
                            $SQL = "insert into coursesuser (coursesid,userid,created_at,status,endtime) values('$lessonId','$auth_id',now(),0,DATE_ADD(now(), INTERVAL $day DAY);)";
                            if (!$dsql->query($SQL)) {
                                echo "  <script >alert('加入失败！')</script>\n";
                            } else {
                                echo "  <script >alert('加入成功')</script>\n";
                            ...
                            echo "  <script >alert('加入人次超过上限 ！')</script>\n";

                        }
                    } else {
                        echo "  <script >alert('加入失败！选课码不对')</script>\n";
                    }
                } else {
                    //加入 课程
                    //第一步 判断 课程 总人次
                    ...
                    if (!$dsql->next_record()) {
                        ...
                        $dsql->next_record();
                        $allcount = $dsql->f("c");
                        if ($student_limit > $allcount || 1==1 ) {//不再做课堂人数的比较
                            $SQL = "SELECT   trueusage,payquantity FROM courses where  id  ='$course_id' and payquantity>trueusage";
                            //echo $SQL . "<br/>";
                            $dsql->query($SQL);
                            if ($dsql->next_record()) {
                                ...
                                if (!$dsql->query($SQL)) {
                                    echo "  <script >alert('加入课堂失败！');window.location='lesson.php?lessonId=$lessonId'</script>\n";
                                } else {
                                    echo "  <script >alert('加入课堂成功');window.location='lesson.php?lessonId=$lessonId'</script>\n";
                                }
                            }else {
                                echo "  <script >alert('加入人次 超过上限 ！')</script>\n";
                            }
                        } else {
                            echo "  <script >alert('加入人次  超过上限  ！')</script>\n";
                        }
                ...
                $dsql->next_record();
                $allcount = $dsql->f("c");
                $dsql->query("update courses set trueusage='$allcount',payquantityusage=$allcount where  id='$lessonId'");
        ...
        $SQL = "SELECT * FROM coursesuser where coursesid='$lessonId'  and  userid = '$auth_id' and status=0";
        //echo $SQL . "<br/>";
        $dsql->query($SQL);
        if ($dsql->next_record()) {
            $iscourseuser = 1;
        }
        $SQL = "SELECT e.* ,ce.starttime as cestarttime,ce.endtime as ceendtime 
        	from experiments e ,coursesexperiment ce  
        	where e.id=ce.experimentsid and ce.coursesid='$lessonId' and ce.COURSE_TYPE=". MetaDataGenerator::COURSE_TYPE_KETANG ." order by ce.sort asc";
        $dsql->query($SQL);
        $i = $offset;
        $i = 0;
        while ($dsql->next_record()) {
            ...
            if ($iscourseuser == 1) {
                if (is_file("$reportpath/xml/test$id.xml")) {
                    $report .= "<li>
    								<div><h4><a href=\"tet.php?id=$id&lessonId=$lessonId\" TARGET='_blank' >实验报告$i <span>$name</span></a></h4></div>
    							</li>";
                }
                ...
                $chapter .= "	<li>
    								...
    								<h4>实验$i $name </h4>
    									...
    									<a class=\"btn btn-primary start_e\"  TARGET='_blank' href=\"experimentplay.php?id=$id&courseid=$lessonId\">开始实验</a>
    								</div>
    							</li>";
            } else {
                ...
                if (is_file("$reportpath/xml/test$id.xml")) {
                    $report .= "<li>
    								<div><h4><a href=\"tet.php?id=$id&lessonId=$lessonId\"  >实验报告$i <span>$name</span></a></h4></div>
    							</li>";
                }
                $chapter .= "	<li>
    								<div class=\"circle\">$i</div>
    								...
    								<h4>实验$i $name </h4>
    									<p>开始:$starttime 结束:$endtime </p>
    									<a class=\"btn btn-primary start_e\"    href=\"experimentplay.php?id=$id&courseid=$lessonId\">开始实验</a>
    								</div>
    							</li>";
            }
        ...
        <div class="ts_btm">
                ...
                <ul class="tab_tar">
                    <li class="tabr3">
                        ...
                        <ul class="ext_con">
                            <div></div>
                            <?
                            $SQL = "SELECT c.id,c.content,c.createtime ,u.userimg,u.truename from  coursesbbs c ,users u  where c.coursesid='$lessonId'  and u.id=c.userid  order by c.id desc";
                            $dsql->query($SQL);
                            $i = $offset;
                            $i = 0;
                            while ($dsql->next_record()) {
            ...
            <div class="tsb_rt">
                ...
                <div class="tsbname"><a href="teacherinfo.php?teacherId=<? print($teacherId) ?>"><? print($truename) ?></a></div>
